...
class OAuthService2
{
    ...
    {
        if (!empty($this->settings['oidcAuthorizeLanguageParameter_b'])) {
            $languageOption = $this->settings['oidcAuthorizeLanguageParameter_b'];
    ...
    {
        $redirectUri = GeneralUtility::getIndpEnv('TYPO3_REQUEST_HOST') . '/typo3conf/ext/cidaas/callback2.php';
        $url = $this->settings['oidcEndpointAuthorize_b'] . '?' . http_build_query([
                'response_type' => 'code',
                'client_id' => $this->settings['oidcClientKey_b'],
                'scope' => $this->settings['oidcClientScopes_b'],
    ...
    {
        if (empty($this->settings['oidcEndpointRevoke_b'])) {
            return false;
        }
        ...
        $request = $provider->getRequest(
            ...
            $this->settings['oidcEndpointRevoke_b'],
            [
                'headers' => [
                    'Authorization' => 'Basic ' . base64_encode($this->settings['oidcClientKey_b'] . ':' . $this->settings['oidcClientSecret_b']),
    ...
    protected function getProvider()
    {
        if ($this->provider === null) {
            ...
            $this->provider = new \League\OAuth2\Client\Provider\GenericProvider([
                'clientId' => $this->settings['oidcClientKey_b'],
                'clientSecret' => $this->settings['oidcClientSecret_b'],
                'redirectUri' => $redirectUri,
                'urlAuthorize' => $this->settings['oidcEndpointAuthorize_b'],
                'urlAccessToken' => $this->settings['oidcEndpointToken_b'],
                'urlResourceOwnerDetails' => $this->settings['oidcEndpointUserInfo_b'],
                'scopes' => GeneralUtility::trimExplode(',', $this->settings['oidcClientScopes_b'], true),
            ]);
        }
